    ...
    <main className="motion-page flex min-h-screen items-center justify-center bg-portal-surface px-6 py-12">
      <section className="flex w-full max-w-[420px] flex-col items-start gap-6 rounded-[10px] border border-portal-border bg-white p-8 shadow-sm">
        ...
        <div className="flex flex-col gap-2">
          <p className="track-section text-body-xs font-medium uppercase text-portal-muted">Demo payment provider</p>
        ...
        {complete ? <Button variant="portalOutlineLg" onClick={() => window.close()}>Close tab</Button> : <Button variant="portalLg" onClick={connect}>Complete fake connection</Button>}
      </section>
    </main>
  );
}
