...
import { saveProfilePhoto, type PhotoCrop, type SavedProfilePhoto } from "../profile-photo";
 ...
 */
function CropModal({ src, onCancel, onApply }: CropModalProps) {
  ...
  const geometry = (scale: number) => {
    // Until the photo loads (it stays hidden), any ratio will do — just not 0.
...
type ProfilePhotoPickerProps = {
  /** Called with the cropped photo once it is applied and saved. */
  onSave: (photo: SavedProfilePhoto) => void;
  /** Renders the screen's own trigger and hint; `error` replaces the hint. */
  children: (picker: { pick: () => void; error: string | null }) => ReactNode;
};

/**
 * The one way to set the profile photo: a hidden JPG/PNG file input, the
 * size and type checks, the crop modal and saving the result. Screens supply
 * only their trigger ("Upload", "Change Photo") and where the hint sits.
 */
export default function ProfilePhotoPicker({ onSave, children }: ProfilePhotoPickerProps) {
  ...
  const onFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > PHOTO_MAX_MB * 1024 * 1024) {
      setError(`This image is over ${PHOTO_MAX_MB} MB. Choose a smaller one.`);
      return;
    }
    setError(null);
  ...
  return (
    <>
      {children({ pick: () => input?.click(), error })}
      <input
        ref={setInput}
        type="file"
