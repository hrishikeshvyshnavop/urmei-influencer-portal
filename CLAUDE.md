...
>
> **Note on the repo name:** the directory and `package.json` are still named `samara` from an earlier marketing-site project. Those files were removed — this repo now contains only the URMEI influencer portal.
...
This is the **URMEI influencer portal** — a set of standalone full-screen auth and onboarding screens implemented from Figma. There is no data layer.
...
- `src/portal/components/` holds the shared pieces:
  - `PortalLayout.tsx` — sticky side accent panel + header + vertically centred content column. Used by every screen except `ApplyLanding` (no header in the design) and `ApplyInfluencer` (own scrolling layout).
